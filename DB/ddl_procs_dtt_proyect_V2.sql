...
/* ---------------------------------------------------------------------------------------------------------------------
                 CONFERENCIAS - Valida las fechas de corte de las prorrogas de las conferencias
-- ---------------------------------------------------------------------------------------------------------------------*/
DROP PROCEDURE IF EXISTS validate_extention_date_conferences;
DELIMITER $$
CREATE PROCEDURE validate_extention_date_conferences()
BEGIN
    DECLARE current_period_id, cur_conference_id INT;
    DECLARE cur_fecha_corte DATETIME;
    DECLARE done1 INT DEFAULT FALSE;
    ...
    DECLARE cursor_active_extentions CURSOR FOR 
        -- SELECT mf.id, mf.estado, mfs.id_periodo, mfe.extention_date
		SELECT mc.id, mce.extention_date
        FROM mdtt_conference mc
        INNER JOIN mdtt_conference_semester mcs ON mc.id_conference_semester = mcs.id
        INNER JOIN (
            SELECT id_conference as id, MAX(extention_date) AS extention_date
            FROM mdtt_conference_extension
            GROUP BY id_conference
        ) mce ON mce.id = mc.id
        WHERE mc.estado_calificacion = 'prorroga'
            AND mcs.id_periodo = current_period_id;
    ...
    OPEN cursor_active_extentions;
    active_extentions: LOOP
        FETCH  cursor_active_extentions INTO cur_conference_id, cur_fecha_corte;
        IF done1 THEN
            LEAVE active_extentions;
        END IF;
