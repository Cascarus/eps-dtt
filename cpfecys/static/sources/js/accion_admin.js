...
function get_items_tabla_post(){
   ...
   $("#tb_reclutamiento >tbody >tr").each(function(){
      ...
      objItem.id = $(this).attr('id').split("-")[1];
      objItem.nota_oposicion = columnas.eq(11).find('input').val();
      objItem.seccion_proyecto = columnas.eq(12).find('select').val();
      objItem.estado = columnas.eq(13).find('select').val();
      objItem.periodos = columnas.eq(14).find('input').val();
      objItem.horas = columnas.eq(15).find('input').val();
   ...
   return JSON.stringify(arrItems);
}

function get_items_tabla_obj(){
