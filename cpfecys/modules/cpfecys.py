...
def _period_setup():
        ...
        # We create the current period  Se regreso a 6 por mmarin
        if now.month < 6:
                #check and create the first semester of current year	
                second_period = db.period(name = second_period_name)
                first_period = db.period(name = first_period_name)	

                pery = db.period_year((db.period_year.yearp == year) & (db.period_year.period == first_period))
                if not pery: db.period_year.insert(yearp = year, period = first_period)

                #check and create the configuration of firts semester of curreynt year
                ...
                if not configPeriod: 
                        ...
                        for d0 in dateInicialP: date1 = d0['fecha']
                        
                        dateInicialP = db.executesql('SELECT date(\''+str(year)+'-02-15\') as fecha;',as_dict=True)
                        for d0 in dateInicialP: date2 = d0['fecha']
                        
                        dateInicialP = db.executesql('SELECT date(\''+str(year)+'-06-30\') as fecha;',as_dict=True)
                        for d0 in dateInicialP: date3 = d0['fecha']
        ...
        if now.month > 6:
                #check and create second semester of current year; se regreso a 6 por mmarin por error en dtt
                first_period = db.period(name = first_period_name)
                second_period = db.period(name = second_period_name)

                pery = db.period_year((db.period_year.yearp == year) & (db.period_year.period == second_period))
                if not pery: db.period_year.insert(yearp = year, period = second_period)
        ...
        if now.month == 12:
                ...
                pery = db.period_year((db.period_year.yearp == (year + 1)) & (db.period_year.period == first_period))
                if not pery:
                ...
                if not configPeriod:
                        ...
                        dateInicialP = db.executesql('SELECT date(\''+str(year+1)+'-06-30\') as fecha;',as_dict=True)
                        for d0 in dateInicialP: date3 = d0['fecha']
...
def _projects_setup():
    ...
    db.project.insert(project_id = '0283B', area_level = lvl_1, name = 'Análisis y Diseño de Sistemas 1 (Sección B)', description = '', physical_location = '')
    ...
    db.project.insert(project_id = '0779N', area_level = lvl_1, name = 'Arquitectura de Computadoras y Ensambladores 2 (Sección N)', description = '', physical_location = '')
    ...
    db.project.insert(project_id = '0720A', area_level = lvl_1, name = 'Modelación y Simulación 2 (Sección A)', description = '', physical_location = '')
    ...
    db.project.insert(project_id = '0281B', area_level = lvl_1, name = 'Sistemas Operativos 1 (Sección B)', description = '', physical_location = '')
...
def force_student_data_update(path, ignore_paths):
    ...
    if _auth.user is not None:
        groups = _db((_db.auth_membership.user_id == _auth.user.id) & (_db.auth_group.role == 'Student')
                & (_db.auth_group.id == _db.auth_membership.group_id)).select().first()
        if groups is None:
            return
...
def current_year_period():
    db = _db
    current_year, current_month = get_date()
    period = second_period

    #current period depends if we are in dates between jan-jun and jul-dec
    if current_month < 6: period = first_period

    return db((db.period_year.yearp == current_year) & (db.period_year.period == period)).select().first()

#emarquez: verifica si un periodo es variable o semestre, enviandolo el ID de period_year.id
def is_semestre(param):
    db = _db
    variable = db((db.period_year.id == param) & (db.period.id == db.period_year.period)
                & (db.period_detail.period == db.period.id)).select()
...
def get_period_from_periodyear(param):
    db = _db
    variable = db(db.period_year.id == param).select()
    
    if variable: return variable[0].period
    return -1

def student_validation_report_owner(report):
    db = _db
    auth = _auth
    usr_rep = db((db.report.id == report) & (db.report.assignation == db.user_project.id)
            & (db.user_project.assigned_user == auth.user.id)).select(db.report.id).first()
    return usr_rep is not None
...
def student_validation_report_status(report):
    ...
    if report.score_date is None:
        if (report.report_restriction.start_date <= current_date and report.report_restriction.end_date >= current_date
            and report.report_restriction.is_enabled and ((report.status.name == 'Draft') or (report.status.name == 'Recheck'))):
            return True
        return False

    next_date = report.score_date + datetime.timedelta(days=get_custom_parameters().rescore_max_days)
    if not assignation_is_locked(report.assignation) and ((report.status.name == 'Draft' and report.report_restriction.is_enabled)
