...
def audit_academic():    
    ...
    grid = SQLFORM.grid(query, exportclasses = None, orderby=db.academic_log.date_log, deletable=False, editable=False, create=False, maxtextlength= 256)
...
#db.user_project.assigned_user == auth.user.id
    ...
    for project_result_item in project_result:
        name_project = project_result_item.name
    ...
    grid = SQLFORM.grid(query,  deletable=False, editable=False, create=False, details=False, maxtextlength=256, csv=False)
...
def audit_register_mail_notifications():
    ...
    return dict(user=user, obtain_name_project=obtain_name_project, obtain_notices=obtain_notices)
...
#db.user_project.assigned_user == auth.user.id
    ...
    if request.vars['area'] != None:
        ...
        def current_teacher(project):
            students = db((db.user_project.project==project)&((db.user_project.period <= period) & ((db.user_project.period.cast('integer') + db.user_project.periods) > period))&(db.auth_membership.user_id==db.user_project.assigned_user)&(db.auth_membership.group_id==3)).select()
            return students
...
#db.user_project.assigned_user == auth.user.id
    ...
    if request.vars['area'] != None:
        ...
        def current_practising(project):
            students = db((db.user_project.project==project)&((db.user_project.period <= period) & ((db.user_project.period.cast('integer') + db.user_project.periods) > period))&(db.auth_membership.user_id==db.user_project.assigned_user)&(db.auth_membership.group_id==2)).select()
            return students
