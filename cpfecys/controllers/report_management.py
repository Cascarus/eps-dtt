...
def student_management():
    ...
    def obtener_total_nivel(tipo, periodo):
        total = 0
        count = db.academic_log.id.count()
        if tipo == "i":
            acciones = (
                db(
                    (db.academic_log.id_period == periodo)
                    & (db.academic_log.operation_log == "insert")
                )
                .select(count)
                .first()
            )
            ...
            acciones = (
                db(
                    (db.academic_log.id_period == periodo)
                    & (db.academic_log.operation_log == "update")
                )
                .select(count)
                .first()
            )
            ...
            acciones = (
                db(
                    (db.academic_log.id_period == periodo)
                    & (db.academic_log.operation_log == "delete")
                )
                .select(count)
                .first()
            )
    ...
    return dict(obtenerSemestres=obtener_semestres, obtenerTotalNivel=obtener_total_nivel)
...
def student_management_n2():
    ...
    if request.vars["semestre"] != None and request.vars["tipo"] != None:
        ...
        count = db.academic_log.id.count()
        var_roles = db(db.auth_group).select()
        for rol in var_roles:
            u = []
            u.append(rol.id)
            u.append(rol.role)
            if tipo == "all":
                acciones = (
                    db(
                        (db.academic_log.id_period == periodo)
                        & (db.academic_log.operation_log == "insert")
                        & (db.academic_log.roll.like("%" + rol.role + "%"))
                    )
                    .select(count)
                    .first()
                )
...
def student_management_n3():
    ...
    rol = None
    if (
        request.vars["semestre"] != None
        and request.vars["tipo"] != None
        and request.vars["rol"] != None
    ):
        periodo = request.vars["semestre"]
        tipo = request.vars["tipo"]
        count = db.academic_log.id.count()
        ...
        if tipo == "all":
            acciones = db(
                (db.academic_log.id_period == periodo)
                & (db.academic_log.roll.like("%" + rol.role + "%"))
            ).select(groupby=db.academic_log.user_name)
                ...
                if usuario != None:
                    ...
                    acciones = (
                        db(
                            ...
                            & (db.academic_log.operation_log == "insert")
                            & (db.academic_log.roll.like("%" + rol.role + "%"))
                            & (db.academic_log.user_name == usuario.username)
                        )
                        .select(count)
                        .first()
                    )
...
def student_management_n4():
        ...
        if request.vars["consulta"] != None:
            ...
            if tipo == "all":
                ...
                operaciones = db(
                    (db.academic_log.id_period == periodo)
                    & (db.academic_log.roll.like("%" + rol.role + "%"))
                    & (db.academic_log.user_name == user.username)
                ).select()
            elif tipo == "i":
                tipo = "i"
                operaciones = db(
    ...
    def obtener_semestre(id):
        return db(db.period_year.id == id).select().first()

    return dict(
        operaciones=operaciones,
        obtenerSemestre=obtener_semestre,
        periodo=periodo,
        tipo=tipo,
        rol=rol.id,
        user=user.id,
